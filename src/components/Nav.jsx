    ...
    return (
        <nav className={`fixed top-0 w-full z-[1000] transition-all duration-500 ${sc ? 'py-2 sm:py-4 bg-white/95 backdrop-blur-xl shadow-[0_4px_24px_rgba(10,22,40,0.08)]' : 'py-3 sm:py-6 bg-transparent'}`}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 flex justify-between items-center">
                ...
                <button className="md:hidden z-50 text-xl sm:text-2xl text-[#0A1628]" onClick={() => sm(!m)}>
                    <i className={`fa-solid ${m ? 'fa-xmark' : 'fa-bars'}`} />
                </button>

                {m && (
                    <div className="fixed inset-0 bg-white z-40 flex flex-col justify-center items-center gap-4 sm:gap-6 md:hidden">
                        {links.map((l, i) => (
                            isInternal(l) ?
